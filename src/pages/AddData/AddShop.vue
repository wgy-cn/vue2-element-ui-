<template>
    <div>
    <!-- 以后遇到这种需要提交后台的页面，优先考虑form结构  除了格式上便利，还有交互功能的考虑 -->
        <HeadTop/>
         ...
         <el-form-item label="营业时间" style="white-space: nowrap;">
          ...
          <el-form-item label="上传店铺头像">
						<el-upload
						  ...
						  <img v-if="formData.image_path" :src="'//elm.cangdu.org/img/' + formData.image_path" class="avatar">
          		...
          		<el-form-item label="上传营业执照">
						<el-upload
						  ...
						  :before-upload="beforeAvatarUpload">
						  <img v-if="formData.business_license_image" :src="'//elm.cangdu.org/img/' + formData.business_license_image" class="avatar">
						  <i v-else class="el-icon-plus avatar-uploader-icon" style=" line-height: 120px;"></i>
						</el-upload>
					</el-form-item>
					<el-form-item label="上传餐饮服务许可证">
						<el-upload
						  ...
						  :before-upload="beforeAvatarUpload">
						  <img v-if="formData.catering_service_license_image" :src="'//elm.cangdu.org/img/' + formData.catering_service_license_image" class="avatar">
						  <i v-else class="el-icon-plus avatar-uploader-icon" style=" line-height: 120px;"></i>
						</el-upload>
					</el-form-item>
...
import HeadTop from '../../components/HeadTop/HeadTop.vue'
   import {cityGuess, addShop, searchplace, foodCategory} from '../../api/api.js'
  import {baseUrl, baseImgPath} from '../../config/env.js' //引入但不使用，用了就报错
export default {
    	data(){
    		return {
    			city: {},
    ...
    methods: {
      ...
      //不知道这个class在哪里定义的，可能在库里，但是没看出奇偶行有何不同
         ...
         submitForm(formName) {
				this.$refs[formName].validate(async (valid) => {
					if (valid) {
						...
						try{//try catch的用途：发现错误但不让程序终止，继续执行之后的语句。此处保留作为样本
